...
function drawDiscoveryMarkers(now) {
  for (const spot of getDiscoverySpots(currentMapId)) {
    ...
    if (!interactive) {
      // Discovery stage: show only a subtle trace, matching the monster
      // discovery flow. The actual item icon stays hidden until interaction zoom.
      ...
      // allowing the click to grant another item until cooldown ends.
...
function drawEntranceLabels() {
  for (const entrance of getMapEntrances(currentMapId)) {
    ...
    ctx.fillStyle = interactive ? "#fff4cf" : "#d6ddd7";
    ctx.font = interactive ? "700 13px system-ui" : "700 12px system-ui";
    ctx.fillText(interactive ? `進入${getMapById(entrance.destinationMapId).name}` : "神秘入口", screenX, screenY + 42);
    ctx.restore();
  }
}
...
function drawDiscoveryLabels(now = Date.now()) {
  for (const spot of getDiscoverySpots(currentMapId)) {
    ...
    if (camera.zoom < interactionZoom) {
      ctx.fillStyle = "rgba(8,25,19,.78)";
