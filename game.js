...
let mapBackgroundAsset = null;
const assetImageCache = new Map();

function getAssetImage(asset) {
  if (!asset) return null;
  if (assetImageCache.has(asset)) return assetImageCache.get(asset);
  const image = new Image();
  const entry = { image, failed: false };
  image.onerror = () => { entry.failed = true; };
  image.src = asset;
  assetImageCache.set(asset, entry);
  return entry;
}

function drawAssetOrEmoji(asset, fallback, size, x = 0, y = 0) {
  if (!asset) {
    ctx.fillText(fallback || "◈", x, y);
    return false;
  }

  const entry = getAssetImage(asset);
  ...
  // A configured formal asset should stay visually empty while loading.
  // Only show the legacy emoji if the image has explicitly failed.
  if (entry.failed) {
    ctx.fillText(fallback || "◈", x, y);
  }
  return false;
...
function syncMapBackground() {
  const asset = currentMap.background?.type === "image"
    ? currentMap.background?.asset
    : null;
  ...
  if (mapBackgroundImage && mapBackgroundAsset === asset) return;

  const image = new Image();
  mapBackgroundAsset = asset;
  mapBackgroundImage = image;
  image.src = asset;
}
...
function getCoverZoom() {
  ...
  return Math.max(w / world.width, h / world.height);
}

function getMinimumZoom() {
  return Math.min(limits.max, Math.max(limits.min, getCoverZoom()));
}

function fitWorldToViewport() {
  const w = Math.max(canvas.clientWidth, 1);
  const h = Math.max(canvas.clientHeight, 1);
  camera.zoom = getMinimumZoom();
...
function preserveCameraOnResize(previousWidth, previousHeight) {
  ...
  camera.zoom = Math.max(getMinimumZoom(), Math.min(limits.max, camera.zoom));
...
function setZoom(zoom, sx = canvas.clientWidth / 2, sy = canvas.clientHeight / 2) {
  ...
  camera.zoom = Math.max(getMinimumZoom(), Math.min(limits.max, zoom));
...
function showDiscoveryToast(title, text, entity, fallback = "✦") {
  if (!discoveryToast) {
    ...
    style.textContent = `
      ...
      .toast-asset{width:42px;height:42px}
  ...
  discoveryToast.querySelector(".discovery-toast-icon").innerHTML = assetIconMarkup(entity, fallback, "toast-asset");
...
function drawTerrain() {
  if (mapBackgroundImage?.complete && mapBackgroundImage.naturalWidth > 0) {
    ctx.drawImage(mapBackgroundImage, 0, 0, world.width, world.height);
    return;
  }

  // A map configured with formal image artwork must never fall back to the
  // old generated terrain while that image is still loading. Paint only a
  // neutral field until the image is ready, preventing a visible flash.
  if (currentMap.background?.type === "image") {
...
function drawMonsterMarkers(now) {
  for (const monster of activeMonsters) {
    ...
    if (found) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      drawAssetOrEmoji(monster.asset, monster.icon || "◈", 30 / camera.zoom, 0, 1 / camera.zoom);
...
function drawDiscoveryMarkers(now) {
  for (const spot of getDiscoverySpots(currentMapId)) {
    ...
    if (found) {
      ...
      drawAssetOrEmoji(item?.asset, item?.icon || "✦", 28 / camera.zoom, 0, 1 / camera.zoom);
      ctx.restore();
      continue;
    }
    ...
    if (!interactive) {
      ...
      ctx.textBaseline = "middle";
      const item = itemById[spot.itemId];
      drawAssetOrEmoji(item?.asset, item?.icon || "✦", 30, 0, 1);
    }
...
function drawEntranceMarkers(now) {
  for (const entrance of getMapEntrances(currentMapId)) {
    ...
    if (found) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      drawAssetOrEmoji(entrance.asset, entrance.icon || "⛰", 28 / camera.zoom, 0, 1 / camera.zoom);
      ctx.restore();
      continue;
    }
    ...
    if (interactive) {
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      drawAssetOrEmoji(entrance.asset, entrance.icon || "⛰", 30, entrance.position.x, entrance.position.y + 1);
    } else {
      ctx.fillStyle = "#fff0ae";
      ctx.beginPath();
...
function drawDiscoveryLabels(now = Date.now()) {
  for (const spot of getDiscoverySpots(currentMapId)) {
    ...
    if (found && remaining > 0) {
      ...
      ctx.fillText(itemById[spot.itemId]?.name || "已發現靈物", screenX, screenY + 36);
      ctx.fillStyle = "rgba(8,25,19,.78)";
      ctx.fillRect(screenX - 48, screenY + 43, 96, 26);
      ctx.fillStyle = "#d6ddd7";
      ctx.font = "700 13px system-ui";
      ctx.fillText(`冷卻 ${formatCooldown(remaining)}`, screenX, screenY + 61);
    } else if (found) {
      ctx.fillStyle = "#fff4cf";
      ctx.font = "700 12px system-ui";
  ...
  collectionGrid.innerHTML = monsters.map(monster => {
    ...
    return `
      <article class="collection-item ${discovered ? "discovered" : "locked"}">
        <div class="monster-icon">${discovered ? assetIconMarkup(monster, "◈", "collection-asset") : "?"}</div>
...
canvas.addEventListener("click", event => {
  ...
  if (target?.type === "entrance") {
    ...
    let entranceDiscoveryChanged = false;
    if (!discoveredEntrances.has(entrance.id)) {
      discoveredEntrances.add(entrance.id);
      entranceDiscoveryChanged = true;
    }

    // The player arrived from sourceMapId, so any entrance in the destination
    // that leads straight back to that source is already known on arrival.
    for (const arrivalEntrance of getMapEntrances(destination.id)) {
      if (arrivalEntrance.enabled === false) continue;
      if (arrivalEntrance.destinationMapId === sourceMapId && !discoveredEntrances.has(arrivalEntrance.id)) {
        discoveredEntrances.add(arrivalEntrance.id);
        entranceDiscoveryChanged = true;
      }
    }
    if (entranceDiscoveryChanged) saveDiscoveredEntrances();

    loadMap(destination.id);
    showDiscoveryToast(`進入：${destination.name}`, `已進入${destination.layer}層地圖`, entrance, entrance.icon || "⛰");
    return;
  }
  ...
  if (target?.type === "monster") {
    ...
    if (!discoveredMonsters.has(monster.id)) {
      ...
      saveDiscovered();
      lastClueMonsterId = null;
      showDiscoveryToast(`發現：${monster.name}`, "已加入收藏圖鑑", monster, monster.icon || "◈");
...
function resizeCanvas() {
  ...
  // ResizeObserver can fire even when the canvas pixel dimensions did not
  // actually change. Reassigning width/height clears the canvas, which looks
  // like a flash while dragging on mobile Safari.
...
canvas.addEventListener("pointermove", event => {
  ...
  if (pointers.size === 2 && pinch) {
    ...
    camera.zoom = Math.max(getMinimumZoom(), Math.min(limits.max, pinch.zoom * scale));
