...
function syncDiscovered() {
  refreshDiscovered();
  // DEV 模式不會由 game.js 寫入 localStorage，所以直接從收藏卡同步。
...
function addItem(itemId) {
  ...
  const next = Math.min(current + 1, item.stackMax);
...
function renderDevButtons() {
  ...
  document.querySelectorAll('[data-dev-count]').forEach(label => {
    ...
    const qty = inventory[item.id] || 0;
    label.textContent = `${item.icon}${item.name} ${qty}/${item.stackMax}`;
  });
}
...
function installUI() {
  ...
  document.addEventListener('click', event => {
    ...
    if (ok) {
      ...
      if (!item || !state || (inventory[itemId] || 0) < 1) return;
  ...
  window.addEventListener('shanhaijing:item-collected', event => {
    ...
    inventory[itemId] = Math.min((inventory[itemId] || 0) + amount, item.stackMax);
    save();
    renderInventory();
  });
  ...
  // 捕捉後 game.js 會重新 renderCollection()；觀察收藏 DOM，讓 DEV 模式也能即時同步妖獸。
