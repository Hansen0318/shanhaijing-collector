...
function addItem(itemId) {
  ...
  // + changes selection only. It NEVER changes real inventory quantity.
...
function openItemUse(itemId) {
  ...
  // The upper panel represents SELECTED items only.
  // An owned-but-unselected item (0/1, 0/2, ...) must never enter the use flow.
  if (!item || !modal || getOwned(itemId) < 1 || getSelected(itemId) < 1) return;

  const targets = targetsFor(item);
  modal.dataset.itemId = itemId;
  modal.querySelector('[data-use-title]').innerHTML = `${assetIconMarkup(item, '✦', 'title-asset')} ${item.name}`;
  ...
  modal.querySelector('[data-use-targets]').innerHTML = targets.length
    ? targets.map(id => {
        ...
        return `<article class="use-target-card">
          <div class="use-target-head"><span class="use-target-icon">${assetIconMarkup(getMonsterById(id), monsterIcons[id] || '◈', 'target-asset')}</span><strong>${monsterNames[id]}</strong></div>
...
function openConfirm(itemId, monsterId, action) {
  ...
  if (!item || !state || !discoveredMonsters.has(monsterId) || getOwned(itemId) < 1 || getSelected(itemId) < 1) return;
  ...
  modal.querySelector('[data-confirm-title]').innerHTML = `${assetIconMarkup(getMonsterById(monsterId), monsterIcons[monsterId] || '◈', 'title-asset')} ${monsterNames[monsterId]}`;
  modal.querySelector('[data-confirm-item]').innerHTML = `${assetIconMarkup(item, '✦', 'title-asset')} ${item.name} ×${getOwned(itemId)}`;
...
function renderInventory() {
  ...
  // Upper panel = items currently selected for use.
  // It must be empty when selected === 0 even if owned > 0.
  const selected = Object.entries(inventory)
    .filter(([id, qty]) => qty > 0 && itemById[id] && getSelected(id) > 0);

  if (count) count.textContent = `${selected.length} 種已選物品`;

  grid.innerHTML = selected.length
    ? selected.map(([id]) => {
        const item = itemById[id];
        return `<button class="inventory-item" type="button" data-item="${id}">
          <span class="inventory-icon">${assetIconMarkup(item, '✦', 'inventory-asset')}</span><strong>${item.name}</strong><small>${getSelected(id)}</small>
        </button>`;
      }).join('')
    : `<div class="inventory-empty">目前沒有選擇要使用的物品。</div>`;
...
function renderSelectorButtons() {
  ...
  document.querySelectorAll('[data-dev-count]').forEach(label => {
    ...
    label.innerHTML = `${assetIconMarkup(item, '✦', 'selector-asset')}${item.name} ${countText(item.id)}`;
  });
}
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .inventory-icon{font-size:30px}.asset-icon{display:inline-grid;place-items:center;vertical-align:middle}.asset-icon img{display:block;width:100%;height:100%;object-fit:contain}.inventory-asset{width:42px;height:42px}.title-asset,.target-asset{width:34px;height:34px}.inventory-item strong{font-size:12px}.inventory-item small{font-size:12px;font-weight:700}
    .inventory-empty,.use-empty{text-align:center;padding:30px 10px;grid-column:1/-1;opacity:.7}
    .dev-section{margin-top:14px;padding-top:12px;border-top:1px dashed rgba(65,53,29,.25)}
    ...
    .dev-title{font-size:.75rem;font-weight:900;color:#6d5a31;margin-bottom:7px}
    .dev-items{display:grid;grid-template-columns:1fr;gap:6px}.dev-item-row{display:grid;grid-template-columns:42px 1fr 42px;gap:5px;align-items:center}.dev-item-row span{display:flex;align-items:center;justify-content:center;min-height:34px;border-radius:8px;background:#fffaf0;color:#3b321f;font-size:.78rem;font-weight:800}.selector-asset{width:28px;height:28px;margin-right:6px}.dev-items button{border:0;border-radius:8px;padding:7px;background:#ded5bc;color:#3b321f;cursor:pointer}.dev-items button:disabled{opacity:.45;cursor:not-allowed}
  `;
...
function installUI() {
  ...
  inventoryModal.innerHTML = `<div class="inventory-card" role="dialog" aria-modal="true">
    ...
    <div class="dev-section"><div class="dev-title">${DEV_MODE ? '🛠 測試模式：' : ''}選擇道具</div><div class="dev-items">${items.map(item => `<div class="dev-item-row"><button type="button" data-dev-item="${item.id}">＋</button><span data-dev-count="${item.id}">${assetIconMarkup(item, '✦', 'selector-asset')}${item.name} 0/0</span><button type="button" data-dev-remove-item="${item.id}">−</button></div>`).join('')}</div></div>
  </div>`;
  ...
  document.addEventListener('click', event => {
    ...
    if (ok) {
      ...
      if (!item || !state || getOwned(itemId) < 1 || getSelected(itemId) < 1) return;
